...
def _resolve_prompt_server():
    if server is None:
        logger.warning("[TS File Browser] PromptServer unavailable. API routes will be disabled.")
        return None
    try:
        return server.PromptServer.instance
    except Exception as exc:
        logger.warning("[TS File Browser] PromptServer init failed. API routes disabled: %s", exc)
        return None
...
def _register_get(path):
    def decorator(func):
        ...
        try:
            ...
            logger.warning("[TS File Browser] Failed to register route '%s': %s", path, exc)
        return func
...
@_register_get("/ts_file_browser/images")
...
@_register_get("/ts_file_browser/thumbnail")
...
@_register_get("/ts_file_browser/view")
...
@_register_post("/ts_file_browser/delete_files")
...
@_register_post("/ts_file_browser/set_ui_state")
...
@_register_get("/ts_file_browser/get_ui_state")
...
@_register_get("/ts_file_browser/pick_file")
