        ...
        return IO.Schema(
            ...
            inputs=[
                ...
                IO.Int.Input(
                    ...
                    max=8192,
                    step=1,
                    tooltip="Exact target width in pixels. 0 = ignore this mode.",
                ),
                IO.Int.Input(
                    "target_height",
                    default=0,
                    ...
                    max=8192,
                    step=1,
                    tooltip="Exact target height in pixels. 0 = ignore this mode.",
                ),
                IO.Int.Input(
                    "smaller_side",
                    default=0,
                    ...
                    max=8192,
                    step=1,
                    tooltip="Resize so the shorter side equals this length in pixels. 0 = off.",
                ),
                IO.Int.Input(
                    "larger_side",
                    default=0,
                    ...
                    max=8192,
                    step=1,
                    tooltip="Resize so the longer side equals this length in pixels. 0 = off.",
                ),
                IO.Float.Input(
                    "scale_factor",
                    default=0.0,
                    ...
                    max=10.0,
                    step=0.1,
                    tooltip="Multiply both dimensions by this factor. 0 = off.",
                ),
                IO.Boolean.Input(
                    "keep_proportion",
                    default=True,
                    tooltip="Keep aspect ratio. With both target_width and target_height set, cover-crops to fill; off distorts to the exact size.",
                ),
                IO.Combo.Input(
                    "upscale_method",
                ...
                IO.Int.Input(
                    ...
                    max=256,
                    step=1,
                    tooltip="Round the final width and height to a multiple of this value.",
                ),
                IO.Float.Input(
                    "megapixels",
                    default=1.0,
                    ...
                    max=256.0,
                    step=0.01,
                    tooltip="Target total pixel count in megapixels. Used only when no other size mode is set.",
                ),
                IO.Boolean.Input(
                    "dont_enlarge",
                    default=False,
                    tooltip="Never upscale above the original size.",
                ),
                IO.Mask.Input(
                    "mask",
                    optional=True,
            ...
            outputs=[
                IO.Image.Output(display_name="IMAGE", tooltip="Resized image."),
                IO.Int.Output(display_name="width", tooltip="Final output width in pixels."),
                IO.Int.Output(display_name="height", tooltip="Final output height in pixels."),
                IO.Mask.Output(display_name="MASK", tooltip="Resized mask (passthrough when no mask is connected)."),
            ],
        )
