    ...
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(
            ...
            inputs=[
                ...
                IO.Mask.Input("mask", tooltip="Mask that defines the region to crop. The bounding box of its non-zero area sets the crop."),
                IO.Int.Input("padding", default=64, min=0, max=320, tooltip="Extra pixels added around the mask bounding box before cropping."),
                IO.Int.Input("divide_by", default=32, min=1, max=64, tooltip="Rounds the crop dimensions up to a multiple of this value (e.g. 32 for VAE-friendly sizes)."),
                IO.Int.Input("max_resolution", default=720, min=320, max=2048, step=1, tooltip="Caps the shorter side of the crop. Larger crops are downscaled to this limit."),
                IO.Int.Input("fixed_mask_frame_index", default=0, min=0, max=9999, step=1, tooltip="1-based frame whose mask defines one fixed crop for the whole batch. 0 = per-frame crop."),
                IO.Int.Input("interpolation_window_size", default=0, min=0, max=100, step=1, tooltip="Smooths crop motion across frames by averaging over this many neighbouring frames. 0 = off."),
                IO.Boolean.Input("force_gpu", default=True, tooltip="Run the crop on GPU when CUDA is available; otherwise falls back to CPU."),
                IO.Boolean.Input("fixed_crop_size", default=False, tooltip="Force every crop to the fixed_width x fixed_height aspect ratio instead of following the mask."),
                IO.Int.Input("fixed_width", default=1024, min=64, max=4096, step=8, tooltip="Target crop width when fixed_crop_size is enabled."),
                IO.Int.Input("fixed_height", default=1024, min=64, max=4096, step=8, tooltip="Target crop height when fixed_crop_size is enabled."),
            ],
            outputs=[
                IO.Image.Output(display_name="cropped_images"),
                IO.Mask.Output(display_name="cropped_masks"),
                _CropData.Output(display_name="crop_data", tooltip="Crop geometry passed to TS Restore From Crop to paste the result back."),
                IO.Int.Output(display_name="width", tooltip="Width of the cropped output."),
                IO.Int.Output(display_name="height", tooltip="Height of the cropped output."),
            ],
        )
