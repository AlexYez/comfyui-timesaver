    ...
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(
            ...
            inputs=[
                ...
                IO.Image.Input("target", tooltip="Frames/images that get recolored toward the reference."),
                IO.Combo.Input("mode", options=_SUPPORTED_MODES, default="mkl", tooltip="mkl is faster and more stable; sinkhorn is more accurate but heavier on memory and time."),
                IO.Combo.Input("device", options=_SUPPORTED_DEVICES, default="auto", tooltip="auto = GPU if available, otherwise CPU. In GPU mode large tensors stay on CPU to guard against OOM."),
                IO.Float.Input("strength", default=1.0, min=0.0, max=1.0, step=0.1, tooltip="Softly blends the original and the color-corrected result."),
                IO.Boolean.Input("enable", default=True, tooltip="When off, the node returns the input target unchanged."),
                IO.Combo.Input("match_mask", options=_SUPPORTED_MASK_MODES, default="none", tooltip="rectangle/ellipse sample only the image border of width mask_size, for stabilization."),
                IO.Int.Input("mask_size", default=32, min=0, max=512, step=1, tooltip="Used only when match_mask is rectangle or ellipse."),
                IO.Int.Input("compute_max_side", default=_DEFAULT_COMPUTE_MAX_SIDE, min=0, max=4096, step=1, tooltip="Compute A,b on a downscaled copy (e.g. 512–1024). Saves memory at 4K with almost no quality loss."),
                IO.Int.Input("mkl_sample_points", default=_DEFAULT_MKL_SAMPLE_POINTS, min=0, max=2000000, step=1, tooltip="200k–500k recommended. At 0 all pixels are used, which can OOM at 4K."),
                IO.Int.Input("sinkhorn_max_points", default=_DEFAULT_SINKHORN_MAX_POINTS, min=0, max=65536, step=1, tooltip="Usually 1024–2048 for 4K. Too large a value can OOM."),
                IO.Boolean.Input("reuse_reference", default=True, tooltip="Speeds up and stabilizes video when one reference is shared across the whole batch."),
                IO.Boolean.Input("temporal_smoothing", default=True, tooltip="EMA smoothing of the color transforms between frames. Enable for video (suppresses flicker); disable for a batch of UNrelated images, otherwise per-frame corrections bleed into each other."),
                IO.Int.Input("chunk_size", default=4, min=0, max=256, step=1, tooltip="4–8 recommended for long 4K videos. 0 processes the whole batch at once."),
                IO.Boolean.Input("logging", default=False, tooltip="When on, logs processing stages, chunk index and GPU memory state to the console."),
            ],
