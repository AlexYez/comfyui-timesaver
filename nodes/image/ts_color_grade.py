    ...
    def define_schema(cls) -> IO.Schema:
        return IO.Schema(
            ...
            inputs=[
                ...
                IO.Float.Input("temperature", default=0.0, min=-1.0, max=1.0, step=0.01, tooltip="Warm/cool balance. Positive shifts toward warm (red), negative toward cool (blue)."),
                IO.Float.Input("saturation", default=1.0, min=0.0, max=3.0, step=0.01, tooltip="Color intensity. 0 = grayscale, 1 = unchanged, >1 = more saturated."),
                IO.Float.Input("contrast", default=1.0, min=0.0, max=3.0, step=0.01, tooltip="Contrast around mid-gray. 1 = unchanged, >1 = more contrast."),
                IO.Float.Input("gain", default=1.0, min=0.0, max=3.0, step=0.01, tooltip="Multiplies pixel values, mainly affecting highlights. 1 = unchanged."),
                IO.Float.Input("lift", default=0.0, min=-1.0, max=1.0, step=0.01, tooltip="Adds a constant offset, mainly raising or lowering shadows. 0 = unchanged."),
                IO.Float.Input("gamma", default=1.0, min=0.1, max=3.0, step=0.01, tooltip="Mid-tone gamma. <1 brightens mid-tones, >1 darkens them. 1 = unchanged."),
                IO.Float.Input("brightness", default=0.0, min=-1.0, max=1.0, step=0.01, tooltip="Adds a constant brightness offset to all pixels. 0 = unchanged."),
            ],
