...
# ==========================================================
# 2пёЏвѓЈ TS Film Emulation Node (V5.0 - ENABLE + РќРћР’Р«Р• Р”Р•Р¤РћР›РўР«)
# ==========================================================
class TS_Film_Emulation:
    ...
    def _resolve_luts_dir(cls):
        ...
        if os.path.isdir(preferred):
            return preferred
        return os.path.join(os.path.dirname(os.path.dirname(__file__)), "luts")
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                "image": ("IMAGE",),
                # <--- РќРћР’Р«Р™ Р§Р•РљР‘РћРљРЎ "ENABLE"
                ...
                # <--- РќРћР’Р«Р• Р—РќРђР§Р•РќРРЇ РџРћ РЈРњРћР›Р§РђРќРР®
    ...
    # Р¤СѓРЅРєС†РёРё РіР°РјРјР°-РєРѕСЂСЂРµРєС†РёРё, РёРґРµРЅС‚РёС‡РЅС‹Рµ OlmLUT, РЅР° Torch
    @staticmethod
    ...
    # Р—Р°РіСЂСѓР·С‡РёРє LUT СЃ РёСЃРїСЂР°РІР»РµРЅРЅС‹Рј РїРѕСЂСЏРґРєРѕРј РѕСЃРµР№
    def load_cube_lut(self, path):
        ...
        lut = lut.permute(2, 1, 0, 3) # <-- РљР»СЋС‡РµРІРѕРµ РёСЃРїСЂР°РІР»РµРЅРёРµ РёРЅРІРµСЂСЃРёРё
        return lut, size

    # РџСЂРёРјРµРЅРµРЅРёРµ LUT
    ...
    # РћСЃС‚Р°Р»СЊРЅС‹Рµ С…РµР»РїРµСЂС‹ Р±РµР· РёР·РјРµРЅРµРЅРёР№
    @staticmethod
    ...
    def process(self, image, enable=True, film_preset="External LUT", lut_choice="None", lut_strength=1.0, gamma_correction=False,
        ...
        # <--- Р›РћР“РРљРђ Р РђР‘РћРўР« Р§Р•РљР‘РћРљРЎРђ ENABLE
        if not enable:
            return (image,)
        ...
        # Р›РћР“РРљРђ РџР РРњР•РќР•РќРРЇ LUT - РўРћР§РќРђРЇ РљРћРџРРЇ OlmLUT
