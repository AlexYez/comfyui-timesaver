...
USER_PRESETS_FILENAME = "ideogram_user_presets.json"  # legacy single-file store, read-only
# User presets live as individual JSON files (one per preset), under per-kind
# subfolders: user_presets/designs/*.json (+ legacy layouts/ and styles/).
...
def load_user_presets() -> dict:
    """User-provided custom layout/style presets, READ-ONLY: the legacy single
    JSON file plus per-file presets dropped into user_presets/<kind>/ by hand
    (the in-editor save/import UI for these was replaced by full-design
    presets). Shape: {"layouts": [...], "styles": [...]}."""
    out = {"layouts": [], "styles": []}
...
def import_design_preset(payload: dict) -> dict | None:
    ...
    if not isinstance(design, dict):
        ...
        if isinstance(payload.get("blocks"), list) or isinstance(payload.get("style"), dict):
            ...
            return None
    # Shape guard: a hand-made file may carry a non-list ``blocks`` or non-dict
    # entries — either would break editor selection/drag after loading. Blocks
    # themselves are stored verbatim (missing ids are re-generated by the JS
    # ``sanitizeBlocks`` on load).
    design = dict(design)
    blocks = design.get("blocks")
    design["blocks"] = [b for b in blocks if isinstance(b, dict)] if isinstance(blocks, list) else []
...
# --------------------------------------------------------------------------- #
_WEIGHT_PHRASE = {"Thin": "thin light weight", "Bold": "bold weight"}
...
def compose_text_desc(block: dict, fonts_by_id: dict[str, dict] | None = None) -> str:
    ...
    snippet = str((preset or {}).get("desc_snippet") or "").strip()
    slots.append(font_desc or snippet or "bold clean sans-serif")
...
def build_caption(design_json: str) -> tuple[str, str]:
    ...
    # here), ref/ref_cleared (preview underlay), style.preset_id & style.font_preset_id and
...
def get_graph_reference(node_id) -> str | None:
    """Filename of the cached graph reference for ``node_id``, or None.

    Falls back to probing the input dir by the deterministic filename, so a
    ...
    try:
        ...
        if os.path.isfile(os.path.join(folder_paths.get_input_directory(), filename)):
            return filename
    except Exception:  # noqa: BLE001 - keep the preview aid best-effort
        return None
    return None
