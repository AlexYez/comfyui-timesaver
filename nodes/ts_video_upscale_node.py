...
import gc
import time
# РџРѕРїС‹С‚РєР° РёРјРїРѕСЂС‚РёСЂРѕРІР°С‚СЊ spandrel
try:
...
class TS_Video_Upscale_With_Model:
    ...
    def _upscale_on_cpu(self, upscale_model, images, upscale_method, new_width, new_height):
        ...
        for i in range(images.shape[0]):
            ...
            print(f"\r|{'в–€' * int(percent/5)}{' ' * (20-int(percent/5))}| {self.step}/{self.steps} [{percent:.1f}%] - {elapsed_str}<{eta_str}", end="", flush=True)
...
# (РљРѕРґ TS_Free_Video_Memory РѕСЃС‚Р°РµС‚СЃСЏ Р±РµР· РёР·РјРµРЅРµРЅРёР№)
