...
# ⚠️ ЭТО СПИСОК ДЛЯ HTTP-МАРШРУТОВ И ВЫБОРА ФАЙЛА, А НЕ ДЛЯ ДЕКОДЕРА.
# ffmpeg распознаёт контейнер ПО СОДЕРЖИМОМУ, а не по расширению: ProRes,
# переименованный в `.zzz`, открывается как ни в чём не бывало (проверено).
# Поэтому сама нода неизвестное расширение не отвергает — решает проба. Список
# нужен там, где расширение единственное, что известно до открытия файла:
# маршрутам (они видны всякому, кто дотянулся до порта) и кнопке выбора файла.
#
# Состав проверен замером: для каждого расширения собран образец и открыт тем же
# PyAV, что и в ноде. Сюда НЕ входят `.braw` и `.r3d` — их ffmpeg не декодирует
# вовсе (закрытые SDK), и запись в список лишь перенесла бы отказ на шаг позже.
VIDEO_EXTENSIONS = frozenset({
    # массовые контейнеры
    ".mp4", ".mov", ".mkv", ".webm", ".avi", ".m4v", ".mpg", ".mpeg",
    ".m2ts", ".mts", ".ts", ".flv", ".wmv", ".ogv", ".gif",
    # монтажные и вещательные: ProRes и DNxHD живут в том числе здесь
    ".mxf", ".qt", ".dv", ".divx",
    ...
    ".y4m", ".nut", ".ivf", ".mjpeg", ".mjpg",
    # анимированные картинки — такое же видео для ffmpeg, как и `.gif`
    ".apng", ".webp", ".avif",
})
