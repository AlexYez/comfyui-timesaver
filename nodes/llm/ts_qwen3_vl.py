        ...
        return IO.Schema(
            ...
            inputs=[
                IO.Combo.Input(
                    ...
                    tooltip="Pick a model from the list. Choose 'Custom (manual)' for a third-party model.",
                ),
                IO.String.Input(
                    "custom_model_id",
                    default="",
                    multiline=False,
                    tooltip="HuggingFace repo id (e.g. 'Qwen/Qwen2-VL-7B-Instruct') or a full local path.",
                ),
                IO.String.Input(
                    "hf_token",
                    default="",
                    multiline=False,
                    tooltip="HuggingFace token (Write/Read) for downloading models. Leave empty for public models. WARNING: saved in the workflow JSON in plain text — do not share a file that contains the token.",
                ),
                IO.Combo.Input(
                    "system_preset",
                    options=preset_options,
                    default=default_preset,
                    tooltip="System-prompt preset. Controls the model's behavior and response style.",
                ),
                IO.String.Input(
                    "prompt",
                    default="",
                    multiline=True,
                    tooltip="Your request (prompt) to the model.",
                ),
                IO.Int.Input(
                    "seed",
                    default=42,
                ...
                IO.Int.Input(
                    ...
                    max=8192,
                    step=64,
                    tooltip="Maximum number of tokens in the response (output length).",
                ),
                IO.Combo.Input(
                    "precision",
                    options=precision_options,
                    default="auto",
                    tooltip="Weight precision. 'auto' picks the best option. int4/int8 require bitsandbytes to be installed.",
                ),
                IO.Combo.Input(
                    "attention_mode",
                    options=attention_options,
                    default="auto",
                    tooltip="Attention implementation. 'flash_attention_2' is faster and more memory-efficient but needs a compatible GPU.",
                ),
                IO.Boolean.Input(
                    "offline_mode",
                    default=False,
                    tooltip="Forbid downloads. Use only files already present in the models/LLM folder.",
                ),
                IO.Boolean.Input(
                    "unload_after_generation",
                    default=False,
                    tooltip="Unload the model from memory right after generation. Saves VRAM but slows down repeated runs.",
                ),
                IO.Boolean.Input(
                    "enable",
                    default=True,
                    tooltip="Enable processing. When disabled, images pass through to the output unchanged.",
                ),
                IO.Int.Input(
                    "max_image_size",
                    default=1024,
                    ...
                    max=4096,
                    step=32,
                    tooltip="Maximum image side length. Larger resolutions require more VRAM.",
                ),
                IO.Int.Input(
                    "video_max_frames",
                    default=16,
                    ...
                    max=256,
                    step=4,
                    tooltip="How many video frames to feed the model. More frames = better context but higher memory use.",
                ),
                IO.Image.Input("image", optional=True, tooltip="Input image."),
                IO.Image.Input("video", optional=True, tooltip="Input video stream (batch of images)."),
                IO.String.Input(
                    "custom_system_prompt",
                    multiline=True,
                    force_input=True,
                    optional=True,
                    tooltip="Your custom system prompt. Used when 'system_preset' is set to 'Your instruction'.",
                ),
            ],
            outputs=[
                IO.String.Output(display_name="generated_text", tooltip="Text generated by the model."),
                IO.Image.Output(display_name="processed_image", tooltip="Input image passed through (unchanged)."),
            ],
        )
