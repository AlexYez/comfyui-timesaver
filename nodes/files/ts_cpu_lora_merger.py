    ...
    def define_schema(cls) -> IO.Schema:
        ...
        return IO.Schema(
            ...
            inputs=[
                ...
                IO.Combo.Input("lora_1_name", options=lora_choices, default=_LORA_NONE, tooltip="First LoRA to bake into the base model. 'None' leaves this slot empty."),
                IO.Float.Input("lora_1_strength", default=1.0, min=-100.0, max=100.0, step=0.01, tooltip="Merge strength for LoRA #1. 1.0 applies it fully; negative values subtract it; 0.0 skips the slot."),
                IO.Combo.Input("lora_2_name", options=lora_choices, default=_LORA_NONE, tooltip="Second LoRA to bake into the base model. 'None' leaves this slot empty."),
                IO.Float.Input("lora_2_strength", default=1.0, min=-100.0, max=100.0, step=0.01, tooltip="Merge strength for LoRA #2. 1.0 applies it fully; negative values subtract it; 0.0 skips the slot."),
                IO.Combo.Input("lora_3_name", options=lora_choices, default=_LORA_NONE, tooltip="Third LoRA to bake into the base model. 'None' leaves this slot empty."),
                IO.Float.Input("lora_3_strength", default=1.0, min=-100.0, max=100.0, step=0.01, tooltip="Merge strength for LoRA #3. 1.0 applies it fully; negative values subtract it; 0.0 skips the slot."),
                IO.Combo.Input("lora_4_name", options=lora_choices, default=_LORA_NONE, tooltip="Fourth LoRA to bake into the base model. 'None' leaves this slot empty."),
                IO.Float.Input("lora_4_strength", default=1.0, min=-100.0, max=100.0, step=0.01, tooltip="Merge strength for LoRA #4. 1.0 applies it fully; negative values subtract it; 0.0 skips the slot."),
                IO.String.Input("output_model_name", default="ts_merged_model.safetensors", multiline=False, tooltip="Filename for the merged model. Saved as safetensors into the checkpoints/diffusion_models folder; a numeric suffix is added if the name already exists."),
            ],
            outputs=[
                IO.String.Output(display_name="log", tooltip="Human-readable merge log: base model, LoRA stack, and save status."),
                IO.String.Output(display_name="saved_model_path", tooltip="Absolute path of the merged safetensors file that was written."),
            ],
        )
