...
const EXTENSION_ID = "ts.superPromptRT";
const NODE_NAME = "TS_SuperPromptRT";
const DOM_WIDGET_NAME = "ts_super_prompt_rt_ui";

const VOICE_ROUTE_BASE = "/ts_super_prompt_rt";
const AI_ROUTE_BASE = "/ts_super_prompt_rt";
const UPLOAD_ROUTE = "/upload/image";
const VOICE_EVENT_PREFIX = "ts_super_prompt_rt_voice";
const AI_EVENT_PREFIX = "ts_super_prompt_rt";
...
const KEEP_LOADED_WIDGET = "keep_loaded";
...
// ⚠️ Одна модель и на речь, и на промпт — она же и то, и другое умеет.
// Поэтому «High quality» здесь не отдельный распознаватель, как у Qwen-ноды с
// Whisper, а выбор ЕДИНОГО артефакта: E2B или E4B.
...
const STYLE_ID = "ts-super-prompt-rt-style";
const STYLE_TEXT = `
...
/* ⚠️ Иконка внутри pill: без этого правила SVG наследовал бы стиль только
   у .ts-sp__btn и рисовался бы в натуральные 24px, ломая высоту строки. */
.ts-sp__pill--icon{padding:0 8px}
.ts-sp__pill svg{width:14px;height:14px;display:block;fill:none;stroke:currentColor;
    stroke-width:2;stroke-linecap:round;stroke-linejoin:round}
/* Группа настроек прижата вправо: слева действия, справа то, что их настраивает. */
.ts-sp__group--settings{margin-left:auto}
...
// Чип памяти: модель остаётся ЛЕЖАТЬ в памяти видеокарты. Булавка,
// стоявшая здесь раньше, означала бы «закрепить в списке» — не то.
// Геометрия симметрична относительно обеих осей: корпус 10×10 по центру,
// по четыре вывода с каждой стороны на одном шаге.
const SVG_ICON_MEMORY = `<svg viewBox="0 0 24 24"><rect x="7" y="7" width="10" height="10" rx="1.5"/><path d="M10 3v4M14 3v4M10 17v4M14 17v4M3 10h4M3 14h4M17 10h4M17 14h4"/></svg>`;
...
const STRINGS = {
    en: {
        aiHqTitle: "Higher-quality model (E4B) for BOTH the prompt and speech recognition. Off: the faster E2B. Downloaded on first use.",
        keepTitle: "Keep the model on the graphics card between runs. Off: it is unloaded when the work is done (about a second) and comes back in roughly five. On: faster repeat presses, but ComfyUI cannot see this memory and will plan its own sampling as if the card were free.",
    ...
    ru: {
        aiHqTitle: "Модель покрупнее (E4B) И для промпта, И для распознавания речи. Выключено — быстрая E2B. Скачивается при первом включении.",
        keepTitle: "Держать модель на видеокарте между запусками. Выключено — она уходит с карты, как только работа сделана (около секунды), и возвращается примерно за пять. Включено — повторные нажатия быстрее, но ComfyUI эту память не видит и спланирует свой сэмплинг как по свободной карте.",
...
function setupSuperPrompt(node) {
    ...
    // ⚠️ Второй кнопки HQ здесь НЕТ, в отличие от старшей ноды. Там их две,
    // потому что распознаёт Whisper, а промпт пишет Qwen — две разные модели,
    // два независимых выбора. Здесь модель ОДНА на оба дела, и второй
    // переключатель того же значения только сбивал бы с толку.
    ...
    aiGroup.append(aiBtn);

    // ---- Settings group: what applies to EVERYTHING the node does. ----
    // ⚠️ HQ живёт здесь, а не при кнопке Enhance. У старшей ноды флажок
    // качества стоит вплотную к действию, потому что там их два и каждый
    // относится к своей модели. Здесь модель одна и на речь, и на промпт —
    // флажок, приклеенный к Enhance, врал бы, что касается только его.
    const settingsGroup = doc.createElement("div");
    settingsGroup.className = "ts-sp__group ts-sp__group--settings";

    const keepToggle = doc.createElement("button");
    keepToggle.type = "button";
    keepToggle.className = "ts-sp__pill ts-sp__pill--toggle ts-sp__pill--icon";
    keepToggle.title = L.keepTitle;
    keepToggle.innerHTML = SVG_ICON_MEMORY;

    // HQ остаётся надписью, а не иконкой: две буквы читаются без наведения, а
    // «качество» рисунком не показать — любой значок здесь пришлось бы
    // расшифровывать тултипом. Та же метка стоит в старшей ноде, и человек,
    // переключившийся между ними, узнаёт её сразу.
    settingsGroup.append(aiHqToggle, keepToggle);
    ...
    // Порядок: [🎤] · [🖼 🖼] · [Enhance] · [пресет ▼] · [HQ | чип]
    //
    // Слева направо — то, что человек ДЕЛАЕТ: наговорить, приложить кадры,
    // улучшить, выбрать под какую модель. Справа, отбитые пустым местом, — две
    // настройки, которые действуют на всё сразу и меняются редко. Пресет
    // растягивается и сам создаёт этот разрыв.
    bar.append(voiceGroup, attachSlots[0].button, attachSlots[1].button,
        aiGroup, presetSelect, settingsGroup);
    ...
    // Оба значения остаются в схеме (иначе не доедут ни до `execute`, ни до
    // сохранённого workflow), но из тела ноды убираются: ими управляют кнопки
    // тулбара, и строкам сверху там не место.
    hideWidget(node, KEEP_LOADED_WIDGET);
    refreshAiHqToggle();
    refreshKeepToggle();
    ...
    function syncHighQualityFromUi() {
        // Значение и так живёт в виджете — переключатель пишет прямо туда.
        setWidgetValue(node, HIGH_QUALITY_WIDGET, aiHqEnabled());
    }
    ...
    function isHighQualityEnabled() {
        return Boolean(getWidgetValue(node, HIGH_QUALITY_WIDGET, false));
    }
    ...
    function keepLoadedEnabled() {
        return Boolean(getWidgetValue(node, KEEP_LOADED_WIDGET, false));
    }

    function refreshKeepToggle() {
        keepToggle.classList.toggle("is-on", keepLoadedEnabled());
        keepToggle.setAttribute("aria-pressed", keepLoadedEnabled() ? "true" : "false");
    }

    keepToggle.addEventListener("click", () => {
        const next = !keepLoadedEnabled();
        const widget = getWidget(node, KEEP_LOADED_WIDGET);
        if (widget) {
            widget.value = next;
            widget.callback?.(next);
        }
        node.properties ||= {};
        node.properties[KEEP_LOADED_WIDGET] = next;
        refreshKeepToggle();
        // Сняли удержание — отдаём карту сразу, а не после следующего прогона.
        if (!next) {
            api.fetchApi(`${AI_ROUTE_BASE}/unload`, { method: "POST" }).catch(() => {});
        }
    });

    aiHqToggle.addEventListener("click", () => {
        // ⚠️ Переключение модели посреди записи или расшифровки рассинхронит
        // то, чем запись будет распознана. Защита переехала сюда со снятой
        // голосовой кнопки — теперь качество одно, и стеречь его надо здесь.
        if (state.isRecording || state.isVoiceBusy) {
        ...
        // Vue-режим (§12.5.13).
        const widget = getWidget(node, HIGH_QUALITY_WIDGET);
        if (widget) {
            widget.value = next;
            widget.callback?.(next);
        }
        node.properties ||= {};
        node.properties[HIGH_QUALITY_WIDGET] = next;
        refreshAiHqToggle();
        // Качество одно на оба дела: распознавание обязано узнать о смене.
        syncActiveVoiceModel();
        refreshStatus();
    });
    ...
    function buildAiPayload(frames) {
        ...
        return {
            ...
            // Один переключатель качества: он же выбирает модель для речи.
    ...
    async function confirmModelDownload(bigger) {
        ...
        try {
            info = await fetchJson(
                `${AI_ROUTE_BASE}/model_status?high_quality=${bigger ? "1" : "0"}`);
        } catch {
    ...
    async function enhancePrompt() {
        ...
        if (!(await confirmModelDownload(payload.high_quality))) return;
