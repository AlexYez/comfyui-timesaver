...
| 🎨 | **[Ideogram](#ideogram)** | 1 | Визуальный дизайнер JSON-промптов для Ideogram 4 — блоки текста/объектов, bbox-раскладка, пресеты стилей и шрифтов, поддержка кириллицы |
...
Визуальный дизайнер JSON-промптов для Ideogram 4. Открывает полноэкранный редактор: перетаскивайте и меняйте размер блоков **текста** и **объектов** на холсте с правильным соотношением сторон (можно поверх референс-изображения), задавайте каждому блоку пресет шрифта и общий стиль. На выходе нода отдаёт валидный **структурированный JSON-капшен** Ideogram 4 как `STRING` плюс выбранное `aspect_ratio` — подаётся в локальный Ideogram-сэмплер или API-ноду. Прямоугольники редактора превращаются в нормализованные bounding-box `[y_min, x_min, y_max, x_max]` (целые 0–1000, начало в левом-верхнем углу). Шрифт задаётся **описанием, а не именем** (у Ideogram нет селектора гарнитуры) — через библиотеку дескрипторных пресетов. Первоклассная поддержка **русского / кириллицы** со встроенными гардами: UPPERCASE + bold по умолчанию, предупреждения о длине / регистре / декоративных шрифтах, постоянный баннер о надёжности и режим *visual-only*, который эмитит чистую плашку, чтобы добавить текст вручную для печати. Резиновое превью в ноде, работающее и в LiteGraph (Nodes 1.0), и в Vue (Nodes 2.0).
