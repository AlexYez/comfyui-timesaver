...
# --- СЛОВАРЬ С ПРЕСЕТАМИ (ДОБАВЛЕН НОВЫЙ ПРЕСЕТ) ---
PRESET_PROMPTS = {
    "Image Edit Command Translation": """You are a specialized AI assistant that translates user requests into precise English commands for AI image editing models. Your goal is to create a command that is clear, direct, and preserves the original image context.

Follow this two-step process:

**Step 1: Translate to English**
First, translate the user's entire request into fluent and accurate English. Keep any English words that are already present.

**Step 2: Reformat as a Precise Command**
Next, restructure the translated English text into a formal editing command using these strict rules:

1.  **Start with a clear action:** The command must begin with a verb like "Replace", "Change", "Add", "Remove", "Make", or "Relight".
2.  **State the target and the change:** Clearly say what to edit and what the new result should be. (e.g., "Change the jacket to red leather", "Replace the sign text with 'SALE'").
3.  **The Golden Rule (Crucial):** After the action, you MUST add a sentence specifying what to preserve. This is the most important part.
    *   Use phrases like: "Keep everything else unchanged."
    *   Or be more specific: "Preserve the original font and perspective."
    *   Or: "Do not alter the background or the subject's pose."

**Final Output:**
Your final output must be ONLY the resulting English editing command. Do not add any comments, greetings, or explanations.""",
...
Your final output must be ONLY the resulting pure English text. Do not add any of your own comments, explanations, or any text other than the translation itself.""",
    "Prompts enhance (Natural Language)": """You are an AI Art Director creating prompts for modern AI image generators.
...
class TS_Qwen2_5_VL_Node:
    ...
    def INPUT_TYPES(cls):
        ...
        return {
            "required": {
                ...
                "repetition_penalty": ("FLOAT", {"default": 1.05, "min": 0.1, "max": 5.0, "step": 0.01}),
    ...
    def _check_model_integrity(self, local_model_path):
        ...
        # Check for either safetensors or pytorch bin index
    ...
    def _load_model(self, model_name, precision, use_flash_attention, offline_mode, hf_token):
        ...
        if offline_mode:
            ...
            if not self._check_model_integrity(local_model_path):
                raise FileNotFoundError(f"Offline mode: Model not found or incomplete at {local_model_path}. Run with 'offline_mode' unchecked once to download.")
        else:
