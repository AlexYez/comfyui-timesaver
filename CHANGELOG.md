...
### TS Latent Upscale: на швах больше не двоится изображение

⚠️ **Результат при том же seed изменится.** Прежние прогоны дословно не
повторятся — картинка на стыках кусков станет другой. Ничего переподключать не
нужно: вход, выходы и их порядок прежние, сохранённые графы открываются как
раньше.

Симптом: после апскейла на склейке кусков видно наложение, будто два кадра
показали разом. Причина оказалась не в склейке. Шум для куска брался по его
форме, а генератор при каждом вызове начинает заново — то есть каждый кусок
получал одинаковый шум, отсчитанный от **своего** начала, и, значит, разный шум
для одного и того же кадра клипа. На перекрытии лежали два несвязанных варианта
одного отрезка времени, и плавный переход честно усреднял их: ровно в середине
шва — полусумма двух разных картинок.

Теперь шум берётся один на весь клип и нарезается теми же окнами. Соседи
стартуют с одинакового шума, приходят почти к одинаковому результату, а переход
сглаживает остаток — ради чего он и был написан. Клип, который влезает в один
кусок, считается ровно как раньше.

`temporal_overlap` можно оставить на 17: перекрытие снова работает как задумано.

### Пак стал понятнее сканеру реестра

Ничего в работе нод не изменилось. Три динамических импорта записаны обычными,
проверка ffmpeg в CI-файле больше не запускает подпроцесс из Python, а Three.js
для TS Angle Select возится неминифицированной сборкой — той же версии r170,
байт в байт с npm, с адресом и sha256 в `ORIGIN.txt` рядом. Минифицированный
файл сканер не мог опознать и находил в его единственной длинной строке сразу
три класса подозрительного кода.

---

## 12.11.1

### TS Audio Loader больше не краснеет после перезагрузки вкладки

Симптом: загрузили трек, сохранили workflow, открыли заново — нода подсвечена
красным, «A required media input has no file selected». Файл при этом лежал на
месте, и прогон бы прошёл: ругался только интерфейс.

Причина — два разных написания одного пути. Список файлов, который нода
публикует, собирался средствами системы, и на Windows файл в подпапке
назывался `ts_audio_loader_recordings\take.wav`, тогда как в сохранённом
workflow он записан так, как его называет весь остальной ComfyUI —
`ts_audio_loader_recordings/take.wav`. Сканер недостающих медиа сравнивает эти
строки буквально и своего же файла не узнавал. Страдал ровно тот, кто записал
дорожку с микрофона или держит аудио в подпапке `input`; трек, лежащий в корне,
проходил.

Теперь список всегда с прямыми косыми, а путь, сохранённый в старом графе,
приводится к тому же виду при открытии — **старые workflow чинятся сами, ничего
переподключать не нужно**. Заодно только что загруженный файл сразу попадает в
список ноды: раньше он появлялся там лишь после перезагрузки страницы.

### Кнопка «в начало лупа» — TS Audio Loader и TS Audio Preview

В ряду транспорта появилась третья кнопка, слева от воспроизведения: она
возвращает курсор к левой границе выделения, а если ничего не выделено — к
началу файла. Звук при этом не прерывается: играет — продолжится с новой точки,
стоит — просто переедет курсор.

---

## 12.11.0

### TS Song Creator — новая нода: текст песни и стиль в одном редакторе

Для моделей генерации песен (**YuE 2**, **ACE-Step**), которым нужны ровно две
строки: слова и описание звучания. Раньше это были две безымянные текстовые
ноды, и вся возня ложилась на человека.

**Теги частей — кнопкой.** `[verse]`, `[chorus]`, `[bridge]` и ещё девять
вставляются нажатием и всегда встают на свою строку. При узкой ноде кнопки
складываются в несколько рядов, а не уезжают за край.

**Ударение — правой кнопкой на гласной.** Выделите гласную (или просто
поставьте на неё курсор) и выберите **Сделать ударной**. К букве добавляется
знак ударения — тот самый, которым пользуются словари. Повторный вызов снимает
его, отдельный пункт убирает все сразу. В кириллице нет ни одной готовой буквы
с акутом, так что руками это делается только через таблицу символов; ради этого
пункта пришлось нарисовать и всё остальное меню — «Копировать», «Вырезать»,
«Вставить», «Выделить всё».

**Библиотека: 55 стилей и 26 голосов.** Стили — от воодушевляющего попа и
рок-гимна до четырёх видов блюза, дрилла, синтвейва, биг-бэнда, кинооркестра и
чиптюна. Голоса отдельным списком: сначала пол, внутри — виды и нюансы, плюс
дуэт, хор, пение всей компанией, вокальные нарезки и «без вокала».

⚠️ **Голос вынесен из жанра сознательно.** Промпт жанра, в котором уже сказано
«female vocal», спорил бы с мужским голосом, выбранным рядом. Поэтому жанр
описывает только жанр, а выбранный голос **заменяет** прежний, а не добавляется
к нему. Ваши правки при этом не затираются: если фрагмент переписан руками,
новый просто дописывается в конец.

Названия и описания переведены, **сами промпты остаются английскими намеренно**:
модель обучалась на этих словах, и перевод ломает обусловливание, а не переводит
его.

**Полноэкранный режим** для длинного текста: те же поля во весь экран,
библиотека рядом.

### Текст в нодах со своим интерфейсом больше не подменяется при открытии workflow

Задевало любую ноду пака, у которой два скрытых текстовых поля и один видимый
список. Симптом: открываете сохранённый workflow, а в поле текста — значение
совсем другого виджета; второе поле пустое.

Причина не в самих нодах. Значения скрытых полей пак хранит по именам, но при
чтении пытался ещё и разобрать старый позиционный формат, а отличал форматы по
тому, какая раскладка «лучше принимает» значения по типам. Строка принимает что
угодно, поэтому два скрытых текстовых поля всегда перевешивали один видимый
список — и сохранение нового формата разбиралось как старое. Теперь формат
определяется точно, по наличию именованных значений; подгонка по типам осталась
только для по-настоящему старых workflow.

---

## 12.10.1

### TS Compare — четыре починки в шторке «до и после»

Все четыре — по замечаниям владельца, и все проверены замерами в браузере.

**Шторка по кадрам налезала на настройки.** В режиме «Nodes 2.0» сравнение
уезжало вверх и накрывало ряды `fps`, `label_a` и `label_b`. Полоса сравнения
лежит вне потока, и ей нужен позиционированный хозяин — в обычном режиме нод он
находился случайно, а в Nodes 2.0 нет. Замерено: шторка вставала на 87…343 при
панели 206…364; теперь совпадает с панелью. Видеошторки это не касалось.

**Имена сторон работали через раз.** Виджеты `label_a` и `label_b` доходили до
видео, но не до пары кадров — там вместо них показывались «A» и «B». Теперь имена
из виджетов стоят в обеих ветках.

**Полоса сравнения у видео стала такой же, как у кадров** — белая линия с
круглой ручкой и стрелками посередине. Раньше у видео была своя полоса без ручки,
и одна нода выглядела по-разному в зависимости от того, кадр пришёл или пачка.

**Полный экран открывался стопкой.** Кнопка разворота живёт в полосе плеера и
уезжала в полноэкранное окно вместе с ней; нажатая там, она открывала ещё одно
окно поверх. Отсюда и «крестик приходится жать три раза» — окон было три. Теперь
внутри окна кнопки нет, второе окно не открывается, а крестик и Esc закрывают с
первого раза.

Сохранённые workflow не задеты: менялся только интерфейс.

---

## 12.10.0

Версия целиком про одно: **что пак разрешает делать чужой стороне**. Поводом
стали вердикты реестра ComfyUI (их удалось получить поимённо, с файлами и
строками), но чинились настоящие дыры, а не статус в витрине. Пользовательские
сценарии не тронуты нигде, кроме одного места, и оно названо первым.

### TS DLSS Upscaler — рантайм больше не приезжает сам

**Что изменилось для вас:** выключатель `download_if_missing` теперь по
умолчанию **выключен**. Раньше достаточно было открыть чужой workflow и нажать
«Run», чтобы на машину приехали 481 МБ чужих проприетарных файлов и запустилась
чужая программа. Согласие на такое должен давать человек, а не значение поля в
графе, который приехал неизвестно от кого.

Включите выключатель один раз — дальше всё как было. Сохранённые workflow не
задеты: своё значение они несут в себе и открываются по-прежнему.

**Каждый файл рантайма теперь сверяется по контрольной сумме, и несовпадение
останавливает ноду.** Раньше оно печатало предупреждение и работа продолжалась.
Разница существенна: `host/nvngx.dll` пак не читает, а **запускает** как
отдельную программу, — а релиз на GitHub можно удалить и залить под тем же
адресом заново. Этот файл, к слову, был единственным, у которого контрольной
суммы не было вовсе.

Если вы сознательно поставили другую сборку рантайма: `TS_DLSS_SKIP_VERIFY=1`.

Попутно исправлена ложная тревога: проверка «воркер больше мегабайта»
срабатывала на исправной установке, потому что настоящий `nvngx.dll` весит 80 КБ.

### Превью видео, звука и картинок — только медиа и только своей странице

Маршруты, которые показывают вам файл по пути с вашего же диска, отвечали любому,
кто дотянулся до порта. Допущение «сервер локальный, значит по ту сторону хозяин
машины» оказалось неверным: запрос на `127.0.0.1:8188` отправляет любая страница,
открытая в браузере, пока вы читаете новости.

Теперь работают два правила, и **ни одно не отнимает произвольный путь** — ради
которого всё и заведено, чтобы не таскать часовые исходники в `input`:

* маршрут отдаёт только медиа — видео, звук, картинки, субтитры. Ни `id_rsa`, ни
  база паролей, ни `extra_model_paths.yaml` не отдаются **никакой** настройкой;
* маршрут отвечает только странице самого ComfyUI.

Второе правило распространено на **все** маршруты пака, а не только на те, что
отдают файлы: чужая страница не прочитает ответ — это запретит браузер, — но
действие произошло бы. Скачивание на десятки гигабайт, загрузка модели на 3 ГБ в
память, удаление рабочих файлов сессии.

Если ComfyUI у вас стоит в обёртке, где страница живёт по другому адресу, чем
сервер (ComfyUI Desktop, чужой прокси), и что-то перестало открываться —
`TS_DISABLE_ORIGIN_GUARD=1`.

### TS Files Downloader — кнопка «Скачать сейчас» стала строже ноды

Нажатая кнопка и запрос, отправленный чужой вкладкой, для сервера выглядят
одинаково, поэтому у кнопки теперь свои правила:

* **папка** — только модельная (или любая внутри `models/`);
* **адрес** — только публичный: `127.0.0.1`, домашняя сеть и `169.254.169.254`
  отвергаются, и редирект, поворачивающий туда посреди загрузки, обрывает её.

Строка, нарушающая правила, отвергается **до первого скачанного байта**, и нода
называет её номер — вместо «загрузка не удалась».

**В графе всё как было.** Абсолютный путь на другой диск, свой NAS на
`192.168.1.50` — нода принимает их по-прежнему: там строку пишет тот, кто сидит
за машиной.

### TS Latent Upscale — чекпойнт читается без исполнения кода

Формат `.ckpt`/`.pt` устроен так, что распаковка файла **может запускать
произвольный код**; этим и опасен чекпойнт, скачанный с форума. Нода читает
только тензоры. Не читается — переведите в `.safetensors` или, если доверяете
именно этому файлу, поставьте `TS_LATENT_UPSCALE_TRUST_PICKLE=1`.

### TS BG Remove (BiRefNet) — код модели только с проверенного коммита

Нода исполняет `birefnet.py`, скачанный с Hugging Face. Репозитории давно
закреплены на конкретные коммиты, но незакреплённый молча откатывался на «свежую
версию, что бы там сегодня ни лежало». Теперь это отказ.

---

## 12.9.1

### TS Audio Loader / TS Audio Preview — нода больше не растёт вниз, когда её сужают

Жалоба: «пытаюсь сузить ноду, а она вместо этого опускается вниз, растёт по
высоте». Замерено перетаскиванием мыши: сужение 880→500 при минимальной высоте
растило TS Audio Loader с 420 до **660** пикселей.

Причины оказалось две, и они независимы.

**Первая — ряды кнопок в панели переносились на вторую строку.** Потолок высоты
интерфейса читает высоту самой ноды, поэтому выросшее содержимое тянуло ноду за
собой. Теперь ряды сужаются вбок и обрезаются, а не растут вниз. Тем же
исправлены **TS Files Downloader** (ряд действий в списке моделей, 233→266) и
**TS LaMa Cleanup** (подпись пустой панели, 170→185).

**Вторая — видна только в обычном режиме нод (Nodes 1.0), и это регрессия не
наша.** Аудио-ноды назначали собственный расчёт размера «для классического
рендерера». Так было можно, пока ComfyUI обслуживал такие интерфейсы двумя
разными путями. Недавние сборки фронт-энда отдают их одним общим путём — и
назначение из безобидного превратилось в источник бесконечного роста. Отсюда же
и странность, которую было видно со стороны: иногда помогало переключить
Nodes 2.0 и обратно. Расчёт убран, обе ноды переведены на общий для пака способ.

Заодно у **TS Audio Preview** в обычном режиме панель теперь занимает всю ноду —
раньше нижний ряд плеера мог не помещаться.

Сохранённые workflow это не задевает: менялись только размеры интерфейса.

---

## 12.9.0

### TS Free VRAM — новая нода: освободить карту в нужный момент
...
Вход `model` подключён — снимается только эта модель, и остальное в графе
остаётся на месте. Не подключён — снимается всё загруженное. В лог пишется,
сколько видеопамяти реально освободилось.

**Зачем это понадобилось.** Тяжёлый VAE Decode у LTX 2.5 обязан лежать в памяти
целиком (в ComfyUI у этого декодера `disable_offload = True`) и просит крупный
резерв. Если рядом всё ещё лежит диффузионная модель, вдвоём они в карту не
помещаются — а на Windows драйвер при этом не сообщает о нехватке памяти, а
начинает выплёскивать её в системную. Внешне это выглядит как зависание:
видеопамять под потолок, GPU 100%, скорость никакая. Исключения нет, поэтому и
штатный переход на тайловый декод не срабатывает. Нода убирает причину —
освобождает карту перед декодом.

⚠️ **Тип провода любой, а место — нет.** ComfyUI выполняет ноду тогда, когда её
результат кому-то нужен, поэтому память освобождает только провод, который
потребляют ПОСЛЕ тяжёлого шага. На проводе, идущем В сэмплер, нода отработает
слишком рано.

Нода намеренно не кэшируется: её работа — побочный эффект, а выход равен входу,
и с обычным отпечатком ComfyUI со второго прогона отдавал бы результат из кэша,
не запуская её вовсе.

---

## 12.8.0

### TS H3 Audio Inject — новая нода: липсинк на MiniMax H3

Закрепляет звуковую дорожку внутри латента, и видео подстраивается под неё.
Нужна, когда персонаж должен произнести именно вашу реплику — озвучку,
записанный голос, песню.

H3 считает видео и звук одним латентом: две части общей последовательности, и
внимание идёт между ними. Нода кодирует дорожку аудио-VAE, кладёт её в звуковую
часть и помечает эту часть сохраняемой. Дальше обычный KSampler на каждом шаге
держит звук неизменным — картинке не остаётся ничего, кроме как совпасть с ним.
Ни модель, ни сэмплер не патчатся: удержание делает штатный механизм
маскированного сэмплинга ComfyUI.

Встаёт между латентом и сэмплером:

```text
MiniMaxH3ImageToVideo -> TS H3 Audio Inject -> KSampler -> VAEDecode
```

Дорожка короче ролика добирается тишиной, длиннее — подрезается. Моно
разводится в стерео. В готовом ролике звучит именно поданная дорожка, а не
похожая на неё.

### TS Audio Loader: точная длительность отдельным выходом

Появился третий выход — `duration_seconds`, дробные секунды. Целый `duration`
округляет ВВЕРХ, и если по нему считать число кадров, ролик выходит длиннее речи
почти на секунду. Выход дописан в конец, слоты `audio` и `duration` остались на
своих местах — сохранённые воркфлоу этого не заметят.

Расчёт длины ролика под дорожку (H3 принимает только `17n + 5` кадров при 24 к/с):

```text
max(5, ceil(a * 24 + 0.2)) + (5 - (max(5, ceil(a * 24 + 0.2)) % 17)) % 17
```

Запас в 0.2 кадра не на глаз: аудио живёт на своей сетке в 40 латентов в секунду,
и слот отклоняется от `кадры / 24` не больше чем на 1/120 секунды. Без запаса
конец фразы иногда подрезается.

### ⚠️ TS Latent Upscale: звук на пересэмплинге больше не денойзится

Апскейл сам является сэмплером и пересэмплирует каждый кусок клипа. Звуковой
поток при этом не закреплялся, то есть модель видела его зашумлённым — а
результат по звуку всё равно выбрасывался, наружу шла исходная дорожка. Для
обычного ролика это была лишняя работа, а для липсинка — потеря синхронизации
ровно на апскейле: губы уезжали от дорожки.

Теперь каждый кусок уходит в сэмплер с маской, которая держит звук неизменным.

**Результат апскейла изменится по сравнению с прежними прогонами**, в том числе
на роликах без липсинка: траектория сэмплинга стала другой. Если под старое
поведение был подобран сид, картинка будет иной.
